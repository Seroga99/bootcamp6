...
const getAllNames = arr => {
    const mass = users.map(elem => elem.name);
    return mass;
};
...
const getUsersByEyeColor = (arr, color) => {
    ...
    let blueEyeColor = users.filter(eyeblue => eyeblue.eyeColor === color);
    return blueEyeColor;
};
...
const getUsersByGender = (arr, gender) => {
    let gendEr = users.filter(el => el.gender === gender)
    .map(el => el.name);
    return gendEr;
};
...
const getUserByEmail = (arr, email) => {
    let userEmail = users.find(el => el.email === email);
    return userEmail;
};
...
const getUsersWithAge = (arr, min, max) => {
    let neededUsers = users.filter(user => user.age > min && user.age < max);
    return neededUsers;
};
...
const getTotalBalance = arr => {
    const balans = users.reduce((acc, sum) => acc + sum.balance, 0);
    return balans;
};
...
const getUsersByFriend = (arr, name) => {
    findFriend = users.map(el => el.friends)
        .filter(fr =>fr === fr[name]);
    return findFriend;
};
